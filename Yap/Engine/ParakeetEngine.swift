...
final class ParakeetEngine: VoiceEngine {
    ...
    private func transcribeAudio(_ samples: [Float], sampleRate: Double) async {
        ...
        do {
            let manager = try await catalog.resolveManager()
            guard !Task.isCancelled else { return }
    ...
    private func writeWAVFile(samples: [Float], sampleRate: Double) throws -> URL {
        ...
        guard let fmt = AVAudioFormat(commonFormat: .pcmFormatFloat32, sampleRate: sampleRate, channels: 1, interleaved: false),
              let pcm = AVAudioPCMBuffer(pcmFormat: fmt, frameCapacity: AVAudioFrameCount(samples.count))
        else { throw CaptureError.emptyAudio }

        pcm.frameLength = AVAudioFrameCount(samples.count)
        samples.withUnsafeBufferPointer {
            pcm.floatChannelData![0].update(from: $0.baseAddress!, count: samples.count)
        }

        let out = try AVAudioFile(forWriting: dest, settings: fmt.settings)
        try out.write(from: pcm)
        return dest
    }
}
